...
const PersonalLoanExplainBlock: React.FC<Props> = ({
  loanAmount,
  loanYears,
  loanMonths,
  interestRate,
  ...
  const Pstr = formatCurrency(loanAmount || 0, currentLocale, currency);
  ...
  return (
    <section className="mt-10">
      <h2 className="mb-3 text-2xl font-extrabold tracking-tight text-center sm:text-left">
        <span className="bg-gradient-to-r from-pink-300 via-fuchsia-300 to-emerald-300 bg-clip-text text-transparent">
          🧾 How Your Personal Loan EMI Is Calculated
        </span>
      </h2>

      <div className="relative rounded-2xl bg-gradient-to-br from-slate-800/90 via-slate-900/90 to-[#0b1220]/90 p-4 sm:p-6 ring-1 ring-pink-500/30 shadow-xl text-[13.5px] sm:text-sm leading-relaxed">
        <div className="pointer-events-none absolute inset-x-0 -top-0.5 h-0.5 bg-gradient-to-r from-pink-500 via-fuchsia-500 to-emerald-500 opacity-60" />

        {/* Formula */}
        <p className="mb-3 text-center font-mono text-[15px] leading-7 text-pink-300">
          EMI = <span className="text-fuchsia-300">[P × r × (1 + r)<sup>n</sup>]</span> ÷ <span className="text-fuchsia-300">[(1 + r)<sup>n</sup> − 1]</span>
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 mb-4">
          <div className="bg-[#0f172a] px-3 py-2 rounded-lg border border-pink-500/20">
          ...
          <div className="bg-[#0f172a] px-3 py-2 rounded-lg border border-emerald-500/20">
            <div className="text-xs text-slate-300">Monthly EMI</div>
            <div className="font-semibold text-white truncate">{emiStr}</div>
          </div>
        </div>
        ...
        <div className="space-y-2 font-mono">
          <div className="flex flex-wrap justify-between">
            <span className="font-semibold text-fuchsia-300">r = APR ÷ 12 ÷ 100</span>
            <span className="text-white">{fmt(interestRate, 4)}% ÷ 12 ÷ 100 = {Number.isFinite(r) ? r.toFixed(8) : "—"}</span>
          </div>
          <div className="flex flex-wrap justify-between">
            <span className="font-semibold text-pink-300">n = Years × 12 + Months</span>
            <span className="text-white">{loanYears || 0}×12 + {loanMonths || 0} = {Math.max(n, 0)}</span>
          </div>
          <div className="flex flex-wrap justify-between">
            <span className="font-semibold text-emerald-300">EMI (monthly)</span>
        ...
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="rounded-lg border border-pink-500/30 bg-pink-500/10 px-3 py-2 text-center">
            <div className="text-pink-300 text-xs uppercase">Principal</div>
...
export default PersonalLoanExplainBlock;
