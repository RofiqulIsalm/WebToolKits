...
// Supabase Client for Guide Image
...
function moneyFmt(value: number, withSymbol: boolean = true): string {
  ...
  if (abs < 9_999_999) return `${sign}${sym}${abs.toLocaleString(undefined, { maximumFractionDigits: 2 })}`;

  let tier = Math.floor(Math.log10(abs) / 3);
  tier = tier >= suffixes.length ? suffixes.length - 1 : tier;
...
const CompoundInterestCalculator: React.FC = () => {
  ...
  // States for Breakdown Mode & Data
  ...
  // Guide Image URL from Supabase
  const [guideImageUrl, setGuideImageUrl] = useState<string>('');

  // Custom Interval Calculations
  const customIntervalDays = useMemo(() => customRate.years * 365 + customRate.months * 30 + customRate.days, [customRate]);

  // Calculate the daily rate based on selected unit
  const getDailyRate = useMemo(() => {
    ...
    switch (rateUnit) {
      ...
      default:
        return (Math.pow(1 + annualRate, 1 / 365) - 1); // Default to daily
    }
  }, [rate, rateUnit]);

  // Time calculation in days
  const totalDays = useMemo(() => timeData.years * 365 + timeData.months * 30 + timeData.days, [timeData]);

  // Effect to load guide image from Supabase
  useEffect(() => {
    const load = async () => {
      try {
  ...
  // Debounce for input changes to avoid recalculating on every keystroke
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      calculateCompoundInterest();
      if (showBreakdown) generateBreakdown();
    }, 220);
    return () => clearTimeout(timeoutId);
  }, [principal, rate, rateUnit, customIntervalDays, totalDays, breakdownMode, includeAllDays, selectedDays, showBreakdown]);
  ...
  // Breakdown Calculation (Daily, Weekly, Monthly, Yearly)
  const generateBreakdown = () => {
    ...
    // Group data for monthly or yearly breakdowns
        ...
        {/* Inputs Section */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-2xl border border-slate-700 bg-slate-900/70 p-6">
              ...
              {/* Principal and Interest Rate Inputs */}
          ...
          <div className="rounded-2xl border border-slate-700 bg-slate-900/70 p-6 flex flex-col justify-between">
            {/* Display Final Amount and Compound Interest */}
              <div>
          ...
          <section className="mt-10 border-t border-gray-700 pt-6 text-slate-300">
              ...
              <div className="mt-8 bg-gradient-to-r from-slate-800/70 via-slate-900/70 to-slate-800/70 rounded-lg border border-slate-700 shadow-inner p-4">
                ...
                <div className="flex flex-wrap gap-3 text-sm">
                  ...
                  >
                    <span className="text-cyan-400">🎯</span> Savings Goal Calculator
                  </Link>
                  ...
                  >
                    <span className="text-indigo-400">🏦</span> FD Calculator
                  </Link>
                  ...
                  >
                    <span className="text-pink-400">📈</span> SIP Calculator
                  </Link>
                </div>
              </div>
            </section>
