 ...
 *
 * Design notes:
 * - Uses Tailwind classes consistent with your glow-card / gradient style
 * - Advanced area is a collapsible section in the same card, with smooth transitions
 * - All advanced features operate on the SAME main "text" state used by Text Counter
 */
...
const TextToolsPage: React.FC = () => {
  ...
  const encodeBase64 = () => {
    try { setCryptoOutput(btoa(unescape(encodeURIComponent(text)))); }
    catch { alert('Invalid input for Base64 encoding'); }
  };
  const decodeBase64 = () => {
    try { setCryptoOutput(decodeURIComponent(escape(atob(text)))); }
  ...
  const analyzeText = () => {
    ...
    const dups = Object.entries(freq).filter(([, c]) => c > 1).map(([w]) => w);
    setDuplicates(dups);
    setFrequencyData(Object.entries(freq).map(([word, count]) => ({ word, count })).sort((a, b) => b.count - a.count));
  };
  ...
  const generateCharStats = () => {
    ...
    setCharStats(Object.entries(freq).map(([char, count]) => ({ char, count })).sort((a, b) => b.count - a.count));
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {selectedTab === 'textCounter' && (
          <div className="glow-card rounded-2xl p-4 sm:p-6 md:p-8 mb-8 relative border border-slate-700/60 bg-slate-800/60 backdrop-blur">
            ...
            <div
              className={`transition-all duration-300 ease-in-out overflow-hidden ${showAdvanced ? 'max-h-[9999px] mt-6' : 'max-h-0'}`}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="rounded-xl border border-yellow-500/30 bg-yellow-900/10 p-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    />
                    <button onClick={encryptText} className="px-3 py-1 rounded bg-yellow-600 hover:bg-yellow-500 text-white">Caesar Encrypt</button>
                    <button onClick={decryptText} className="px-3 py-1 rounded bg-yellow-700 hover:bg-yellow-600 text-white">Caesar Decrypt</button>
                    <button onClick={encodeBase64} className="px-3 py-1 rounded bg-green-600 hover:bg-green-500 text-white">Base64 Encode</button>
                    <button onClick={decodeBase64} className="px-3 py-1 rounded bg-green-700 hover:bg-green-600 text-white">Base64 Decode</button>
                  </div>
                  <textarea
                    value={cryptoOutput}
                    readOnly
                  ...
                  <div className="flex justify-end gap-2 mt-2">
                    <button onClick={() => copyTextToClipboard(cryptoOutput)} className="text-xs bg-teal-600 hover:bg-teal-500 text-white px-3 py-1 rounded">{copied ? 'Copied!' : 'Copy'}</button>
                    <button onClick={() => downloadTextFile(cryptoOutput, 'crypto-output.txt')} className="text-xs bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1 rounded">Download</button>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-green-500/30 bg-green-900/10 p-4 md:col-span-2">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    <button onClick={analyzeText} className="px-3 py-1 rounded bg-green-600 hover:bg-green-500 text-white">Analyze Duplicates & Frequency</button>
                    <button onClick={generateCharStats} className="px-3 py-1 rounded bg-blue-600 hover:bg-blue-500 text-white">Character Distribution</button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="rounded-lg border border-yellow-500/30 bg-yellow-900/5 p-3">
                      <h4 className="text-sm font-semibold text-yellow-300 mb-2">🔁 Duplicate Words</h4>
                      {duplicates.length > 0 ? (
                        <p className="text-slate-200">{duplicates.join(', ')}</p>
                      ) : (
                        <p className="text-slate-400">No duplicates detected yet. Run analysis to see results.</p>
                      )}
                    </div>

                    {/* Word Frequency */}
                    <div className="rounded-lg border border-green-500/30 bg-green-900/5 p-3">
                      <h4 className="text-sm font-semibold text-green-300 mb-2">📊 Word Frequency</h4>
                        ...
                        <div className="max-h-56 overflow-y-auto">
                          <table className="w-full text-left text-slate-200 text-sm">
                            <thead className="text-slate-400 sticky top-0 bg-green-950/40">
                              <tr><th className="py-1 pr-2">Word</th><th className="py-1">Count</th></tr>
                            </thead>
                            <tbody>
                  ...
                  <div className="mt-4 rounded-lg border border-violet-500/30 bg-violet-900/10 p-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h4 className="text-sm font-semibold text-violet-200">🧠 Regex Tester & Similarity</h4>
                    </div>
                    <div className="flex flex-wrap gap-2 items-center mb-2">
                      ...
                      />
                      <button onClick={runRegexTest} className="px-3 py-1 rounded bg-violet-600 hover:bg-violet-500 text-white">Test Regex</button>
                      {regexMatches.length > 0 && (
                        <span className="text-green-300 text-sm">✅ {regexMatches.length} match(es)</span>
                      )}
                    </div>
                    {regexMatches.length > 0 && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        />
                        <button onClick={computeSimilarity} className="mt-2 px-3 py-1 rounded bg-yellow-600 hover:bg-yellow-500 text-white">Compute Similarity</button>
                        {similarityScore > 0 && (
                          <p className="mt-2 text-slate-200 text-sm">
                            Similarity Score: <span className="text-green-400 font-semibold">{similarityScore}%</span>
                          </p>
                        )}
                      </div>

                      <div>
                        <label className="text-sm text-slate-300">Character Distribution</label>
                          ...
                          {charStats.length > 0 ? (
                            <table className="w-full text-left text-slate-200 text-sm">
                              <thead className="text-slate-400 sticky top-0 bg-slate-900/70">
                                <tr><th className="py-1 px-2">Char</th><th className="py-1">Count</th></tr>
                              </thead>
                              <tbody>
                                {charStats.map((c, i) => (
                                  <tr key={i} className="odd:bg-slate-800/40">
                            ...
                            <div className="p-3 text-slate-400 text-sm">Click "Character Distribution" above to generate stats.</div>
                          )}
                        </div>
                      </div>
                    </div>
        ...
        {/* --------- Existing Informational Section (left as-is, minor cleanup) --------- */}
        <div className="rounded-2xl p-8 mb-8">
          <h2 className="text-3xl font-bold text-white mb-4">📝 Text Counter – Count, Convert & Generate Text Instantly</h2>
          <h3 className="text-xl text-slate-200 mb-2">What is a Text Counter and Why Do You Need It?</h3>
          <div className="space-y-4 text-slate-300">
            <p>
              A <strong>Text Counter Tool</strong> helps writers, students, developers, and digital marketers quickly analyze and manage their content.
              Whether you’re counting characters for a tweet, checking word density for SEO, or formatting a long document, this free online <strong>text counter</strong> does it all in seconds.
            </p>
            <p>
            ...
            <h2 className="text-yellow-500 font-semibold text-lg"><strong>What is a QR Code?</strong></h2>
            <p>
            ...
            <h2 className="text-yellow-500 font-semibold text-lg"><strong>What is a Barcode?</strong></h2>
            <p>
            ...
            <h2 className="text-yellow-500 font-semibold text-lg"><strong>What is a Hash Code?</strong></h2>
            <p>
              ...
              <div className="space-y-4 text-lg text-slate-100 leading-relaxed">
                <div>
                  <div className="bg-slate-800/60 p-4 rounded-lg">
                      ...
                      <p>It helps you count characters, words, and lines in any text to stay within writing limits.</p>
                  </div>
                </div>
                <div>
                  <div className="bg-slate-800/60 p-4 rounded-lg">
