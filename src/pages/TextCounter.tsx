...
const TextToolsPage: React.FC = () => {
  ...
  const reverseText = (mode: 'word' | 'sentence' | 'line') => {
    ...
    setText(reversed);
    setReverseDropdownOpen(false); // Close dropdown after selecting
  };
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-2 md:px-0">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          <button className={`px-4 py-2 rounded-xl font-semibold ${selectedTab === 'textCounter' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300'}`}
            onClick={() => setSelectedTab('textCounter')}>Text Counter</button>
          <button className={`px-4 py-2 rounded-xl font-semibold ${selectedTab === 'loremIpsum' ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300'}`}
            onClick={() => setSelectedTab('loremIpsum')}>Lorem Ipsum Generator</button>
        </div>

        {/* ----------------- Text Counter ----------------- */}
        {selectedTab === 'textCounter' && (
          <div className="glow-card rounded-2xl p-4 md:p-8 mb-8 relative">
            <div className="flex items-center space-x-3 mb-4">
              <FileText className="h-8 w-8 text-blue-400" />
              <h1 className="text-2xl md:text-3xl font-bold text-white">Text Counter</h1>
            </div>

            {/* Paste Button */}
            <button onClick={async () => {
              try { const clipText = await navigator.clipboard.readText(); setText(clipText); } 
              catch { alert('Failed to read clipboard — please allow clipboard access.'); }
            }} className="absolute top-3 right-3 text-sm text-blue-400 hover:text-blue-300 transition-colors bg-slate-800/70 px-3 py-1 rounded-md border border-slate-600">Paste</button>
            ...
            {/* ----------------- Tools: Reverse + Convert + Copy/Download/Clear ----------------- */}
            <div className="flex flex-wrap justify-between items-center gap-2 mt-2">

              {/* Reverse Text Dropdown */}
              <div className="relative">
                <button onClick={() => setReverseDropdownOpen(!reverseDropdownOpen)}
        ...
        {selectedTab === 'loremIpsum' && (
          <div className="glow-card rounded-2xl p-4 md:p-8 mb-8 relative">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <h1 className="text-2xl md:text-3xl font-bold text-white">Lorem Ipsum Generator</h1>
            </div>

            {/* ----------------- Controls ----------------- */}
            <div className="flex flex-wrap gap-2 mb-4">
              <input type="number" min={1} value={paragraphsCount} onChange={e => setParagraphsCount(Number(e.target.value))}
              ...
              <button onClick={generateLoremIpsum} className="text-xs bg-green-600 hover:bg-green-500 text-white px-3 py-2 rounded transition">Generate</button>
            </div>

            <textarea value={loremText} readOnly
            ...
            <div className="flex flex-wrap justify-between items-center mt-2 gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2 relative">
                {/* Convert Case Dropdown */}
                <div className="relative">
                  <button onClick={() => setLoremDropdownOpen(!loremDropdownOpen)}
