...
const Homepage: React.FC = () => {
  // Memoize derived data so it isn't recalculated on every render
  const popularCalculators = useMemo(() => getPopularCalculators(), []);
  const totalToolsCount = useMemo(
    () => toolsData.reduce((total, category) => total + category.tools.length, 0),
    []
  );
    ...
    <>
      ...
      <main className="space-y-12">
        ...
        <section className="text-center mb-12">
          ...
          <div className="flex flex-wrap items-center justify-center text-sm text-slate-300 gap-4 md:space-x-6">
        ...
        {/* CTA Section */}
        <section className="mt-16 glow-card rounded-lg p-8 text-center">
          <h3 className="text-xl font-semibold text-white mb-2">
            Need a specific calculator?
          </h3>
          <p className="text-slate-300 mb-4">
            Browse our complete collection of {totalToolsCount} calculators and tools.
          </p>
