...
const SpeedConverter: React.FC = () => {
  ...
  const units = [
    { key: 'ms', name: 'Meters per Second (m/s)', factor: 1 },
    { key: 'kmh', name: 'Kilometers per Hour (km/h)', factor: 0.277778 },
    { key: 'mph', name: 'Miles per Hour (mph)', factor: 0.44704 },
    { key: 'fps', name: 'Feet per Second (ft/s)', factor: 0.3048 },
    { key: 'knots', name: 'Knots (kn)', factor: 0.514444 },
    { key: 'mach', name: 'Mach', factor: 343 }
  ];
  ...
  const convertSpeed = () => {
    ...
    const valueInMS = value * fromUnitData.factor;
    const newResults: {[key: string]: number} = {};

    units.forEach(unit => {
      if (unit.key !== fromUnit) {
        newResults[unit.key] = valueInMS / unit.factor;
      }
    });
